...
**Rate limiting**: `lib/rate-limit.ts` — one `enforceRateLimit(req, bucket, limit, windowMs)` interface, in-memory sliding window by default. SERVERLESS CAVEAT: per-instance on Vercel (advisory, not global); swap the backend to Upstash/KV for a hard global limit — config, not refactor. Applied to the unauthenticated proxies (salt, zkproof, sponsor, submit-tx, auth/session mint).

**Session auth (signed cookie)**: `cashpan-sub` is an HMAC-SHA256 sealed token `base64url({sub,exp}).sig` under `SESSION_SECRET`, NOT the raw Google sub (which was forgeable — CDP finding). Minted only after server-side Google id_token verification (`lib/google-id-token.ts`, RS256/JWKS); a client-claimed sub is never trusted. EVERY authed route verifies through the one helper `getAuthedSub(req)` / `verifySessionCookie` in `lib/session.ts` — missing/forged/expired → clean 401. `SESSION_SECRET` must be set (Vercel env); rotating it, or this deploy itself, invalidates all existing sessions (one re-login, no dual-acceptance window).
